...
describe('server responses', () => {
  ...
  it('should respond to a GET request for a swim command', (done) => {
     let {req, res} = server.mock('/random', 'GET', 'left');
  ...
  xit('should respond to a POST request to save a background image', (done) => {
    fs.readFile(postTestFile, (err, fileData) => {
      ...
      let {req, res} = server.mock('/random', 'POST', fileData);
  ...
  xit('should send back the previously saved image', (done) => {
    fs.readFile(postTestFile, (err, fileData) => {
      httpHandler.backgroundImageFile = path.join('.', 'spec', 'temp.jpg');
      let post = server.mock('/random', 'POST', fileData);

      httpHandler.router(post.req, post.res, () => {
        let get = server.mock('/random', 'GET');
